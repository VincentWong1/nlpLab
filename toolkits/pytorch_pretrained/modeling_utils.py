...
class PreTrainedModel(nn.Module):
    ...
    def resize_token_type_embeddings(self, new_num_tokens=None):
        """ Resize input token embeddings matrix of the model if new_num_tokens != config.type_vocab_size.
        Take care of tying weights embeddings afterwards if the model class has a `tie_weights()` method.
        ...
        """
        base_model = getattr(self, self.base_model_prefix, self)  # get the base model if needed
        model_embeds = base_model._resize_token_type_embeddings(new_num_tokens)
        if new_num_tokens is None:
            return model_embeds

        # Update base model and current model config
        self.config.type_vocab_size = new_num_tokens
        base_model.type_vocab_size = new_num_tokens
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        # set token types to 3: tol, usr, svr
        model.resize_token_type_embeddings(new_num_tokens=3)
