...
def collate_fn(batch):
    ...
    all_input_ids, all_attention_mask, all_token_type_ids, all_lens, all_labels, all_nums = map(torch.stack, zip(*batch))
    max_len = max(all_lens).item()
    all_input_ids = all_input_ids[:, :max_len]
    all_attention_mask = all_attention_mask[:, :max_len]
    all_token_type_ids = all_token_type_ids[:, :max_len]
    return all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_nums
...
class SmartBoostNumProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_a = line[0]
            label = line[-1]
            num = [float(n) for n in line[1:-1]]
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=None, num=num, label=label))
        return examples
